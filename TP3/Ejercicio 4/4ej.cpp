...
void resolver(int n, int m, int chabones, vector<vector<int>> &listAdy, vector<vector<int>> &capacidades) {
    ...
    while (low + 1 < max) {
        ...
        /*
        if (flujo < chabones) max = mid -1;
        else{
            low = mid +1;
            if(maxactual < mid) maxactual = mid;
        }
        */
...
int main() {
	...
	ifstream fin("./4ej_input.txt");
	if (!fin.is_open()) {
		cerr << "Error: could not open input file 4ej_input.txt" << endl;
		return 1;
	}
	...
	while(tests--) {
		...
		int n, m, chabones;
		fin >> n >> m >> chabones;     // Esquinas, calles, trabajadores
		...
		for (int i = 0; i < m; i++) {
            ...
            //listAdy[w-1].push_back(v-1);
			capacidades[v-1][w-1] = c; //c es la capacidad de la calle v->w, a priori las calles son dirigidas

		}

		maxHerramientas /= chabones;
		// TODO: Ver si armamos el GRAFO en vez del DIGRAFO. No lo hacemos en el for() porque quizás recibimos
		//  el v->w y w->v en  el test (unos rancios)
